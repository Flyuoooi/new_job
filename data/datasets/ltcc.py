...
class LTCC(object):
    ...
    def _process_dir_test(self, query_path, gallery_path):
        ...
        query_dataset = []
        gallery_dataset = []
        # images_info_query = []  # 这两个变量在函数末尾并未返回，实际上是多余的，可以注释掉
        # images_info_gallery = []
        ...
        for img_path in query_img_paths:
            ...
            # 原代码这里有一行无缩进的 images_info_query.append，会导致 KeyError，已移除
        ...
        for img_path in gallery_img_paths:
            ...
            # 原代码这里有一行无缩进的 images_info_gallery.append，会导致 KeyError，已移除
