...
class PRCC(object):
    ...
    def __init__(self, root='data',aux_info=False,meta_dir='PAR_PETA_105.txt',meta_dims=105, **kwargs):
        ...
        train, num_train_pids, num_train_imgs, num_train_clothes, pid2clothes = \
            self._process_dir_train(self.train_dir,aux_info)
        val, num_val_pids, num_val_imgs, num_val_clothes, _ = \
            self._process_dir_train(self.val_dir,aux_info)

        query_same, query_diff, gallery, num_test_pids, \
            num_query_imgs_same, num_query_imgs_diff, num_gallery_imgs, \
            num_test_clothes, gallery_idx = self._process_dir_test(self.test_dir,aux_info)
    ...
    def _process_dir_train(self, dir_path,aux_info):
        ...
        if aux_info:
            ...
            # print(imgdir2attribute['/home/gpu/data/BY/data/prcc/rgb/train/332/B_cropped_rgb103.jpg'])
        ...
        for pdir in pdirs:
            ...
            for img_dir in img_dirs:
                ...
                if aux_info:
                    ...
                    # print(imgdir2attribute[img_dir])
                    dataset.append((img_dir, label, camid, clothes_id, imgdir2attribute[img_dir]))
                    images_info.append({'attributes': imgdir2attribute[img_dir]})
                else:
                    if cam in ['A', 'B']:
                        clothes_id = clothes2label[osp.basename(pdir)]
                    else:
    ...
    def _process_dir_test(self, test_path,aux_info):
                ...
                for img_dir in img_dirs:
                    ...
                    if aux_info:
                        if cam == 'A':
                            ...
                            gallery_dataset.append((img_dir, pid, camid, clothes_id,imgdir2attribute[img_dir]))
                            images_info.append({'attributes': imgdir2attribute[img_dir]})
                        elif cam == 'B':
                            clothes_id = pid2label[pid] * 2
                            query_dataset_same_clothes.append((img_dir, pid, camid, clothes_id,imgdir2attribute[img_dir]))
                            images_info.append({'attributes': imgdir2attribute[img_dir]})
                        else:
                            clothes_id = pid2label[pid] * 2 + 1
                            query_dataset_diff_clothes.append((img_dir, pid, camid, clothes_id,imgdir2attribute[img_dir]))
                            images_info.append({'attributes': imgdir2attribute[img_dir]})
                    else:
                        if cam == 'A':
                            clothes_id = pid2label[pid] * 2
        ...
        pid2imgidx = {}
        if aux_info:
            for idx, (img_dir, pid, camid, clothes_id,attribute_id) in enumerate(gallery_dataset):
